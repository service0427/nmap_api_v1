...
async def lifespan(app: FastAPI):
    logger.info("Starting up API Server v1.1: initiating background monitoring task.")
    monitoring_task = asyncio.create_task(log_system_metrics())
    yield
    logger.info("Shutting down API Server v1.1: canceling background monitoring task.")
...
app = FastAPI(title="Nmap Production API v1.1", lifespan=lifespan)
