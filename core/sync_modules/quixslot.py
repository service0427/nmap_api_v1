...
API_URL = "https://quixslot.com/api/external/work"
...
def fetch_data():
    ...
    try:
        ...
        print(f"[QUIXSLOT] Fetching data from: {url}")
        ...
        if isinstance(res_data, list):
            ...
            print(f"[QUIXSLOT] Unexpected response type: {type(res_data)}")
            return []

        if not raw_items:
            print("[QUIXSLOT] API returned 0 items.")
            return []
        ...
        print(f"[QUIXSLOT] Successfully processed {len(standardized_data)} slots.")
