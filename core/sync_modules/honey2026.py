...
API_URL = "https://honey2026.com/api/external/work-details"
DB_NAME = "honey2026"
...
def get_default_work_amount():
    ...
    try:
        ...
        try:
            ...
            print(f"[HONEY2026] Failed to fetch default_work_amount from DB ({e2}). Falling back to 5.")
    return 5

def fetch_data():
    """
    HONEY2026 API(/api/external/work-details)로부터 데이터를 수집하여 표준 형식으로 반환.
    """
    try:
        ...
        while True:
            ...
            print(f"[HONEY2026] Fetching page {page} from {url}...")
            resp = requests.get(url, timeout=15)
            if resp.status_code != 200:
                print(f"[HONEY2026] HTTP Error: {resp.status_code}")
                break
            ...
            print(f"[HONEY2026] Page {page}: Got {len(items)} items. Total: {len(all_items)}")
        ...
        print(f"[HONEY2026] Successfully processed {len(standardized_data)} slots.")
        return standardized_data

    except Exception as e:
        print(f"[HONEY2026] Fetch Exception: {e}")
        return []
