...
def fetch_data():
    ...
    try:
        ...
        print(f"[LUF] Fetching data from: {url}")
        ...
        if not isinstance(data_list, list):
            print(f"[LUF] Invalid API response format: expected list, got {type(data_list)}")
            return None
            
        if len(data_list) < 500:
            print(f"[LUF] Sync aborted: API returned only {len(data_list)} items (expected >= 500). Treating as temporary failure to retry.")
            return None
