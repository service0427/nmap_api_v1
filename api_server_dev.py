...
# Import the dev-test allocation router instead of the production one
from api.allocation_dev import router as allocation_router
...
async def lifespan(app: FastAPI):
    logger.info("Starting up API Server Dev-Test: initiating background monitoring task.")
    monitoring_task = asyncio.create_task(log_system_metrics())
    yield
    logger.info("Shutting down API Server Dev-Test: canceling background monitoring task.")
...
app = FastAPI(title="Nmap Developer Test API v1.1", lifespan=lifespan)
...
# Mount APIRouters (using allocation_dev as allocation_router)
...
if __name__ == "__main__":
    import uvicorn
    # TODO(security): Listen on 0.0.0.0 is required for external dev-client device connections. Ensure firewall rules restrict access.
    # Runs on port 8011 by default, or can be customized via PORT env variable.
    port = int(os.environ.get("PORT", 8011))
    logger.info(f"Starting dev test server on port {port}")
